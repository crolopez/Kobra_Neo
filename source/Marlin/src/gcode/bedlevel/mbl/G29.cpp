...
void GcodeSuite::G29() {
  ...
  switch (state) {
    ...
    case MeshNext:
      ...
      if (mbl_probe_index < (GRID_MAX_POINTS)) {
        ...
        xy_pos_t pos;
        pos.x = bedlevel.index_to_xpos[ix];
        pos.y = bedlevel.index_to_ypos[iy];
        _manual_goto_xy(pos);
      }
      else {
    ...
    case MeshSet:
      if (parser.seenval('I')) {
        ...
        if (!WITHIN(ix, 0, (GRID_MAX_POINTS_X) - 1)) {
          SERIAL_ECHOLNPGM_3("I out of range (0-", (GRID_MAX_POINTS_X) - 1, ")");
          return;
        }
      }
      else
      ...
      if (parser.seenval('J')) {
        ...
        if (!WITHIN(iy, 0, (GRID_MAX_POINTS_Y) - 1)) {
          SERIAL_ECHOLNPGM_3("J out of range (0-", (GRID_MAX_POINTS_Y) - 1, ")");
          return;
        }
      }
      else
  ...
  if (state == MeshNext) {
    SERIAL_ECHOLNPGM_4("MBL G29 point ", _MIN(mbl_probe_index, GRID_MAX_POINTS), " of ", GRID_MAX_POINTS);
